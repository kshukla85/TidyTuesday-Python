...
For question 3, I am counting by `patient_id` and I am filtering by `start_date`. Then, I joined the tables `treatment` and `diagnosis` to get treatment in 2020 and after. [2020_patient_treatment](2020_patient_treament.sql)

For question 4, I took the avg of `survival_months`, and then the avg by `stage`. Also, I joined the tables `outcomes` and `diagnosis` to get the average survival months. [avg_survival_months](avg_survival_months.sql)
